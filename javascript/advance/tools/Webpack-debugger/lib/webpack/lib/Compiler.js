...
class Compiler extends Tapable {
	...
	run(callback) {
		...
		console.log('  2.1.============================> [生命钩子]compiler.run 方法首先执行beforRun 生命钩子')
		/*
		在NodeEnvironmentPlugin 插件中注册了beforeRun 钩子， 绑定了读取文件系统的对象。
		...
		*/
		this.hooks.beforeRun.callAsync(this, err => {
			if (err) return finalCallback(err);
			console.log('  2.2.============================> [生命钩子]compiler.run 方法首先执行run 生命钩子')
			this.hooks.run.callAsync(this, err => {
				if (err) return finalCallback(err);

				this.readRecords(err => {
					if (err) return finalCallback(err);
					console.log('  2.3.============================> 执行compile方法')
					this.compile(onCompiled);
				});
			});
		});
	...
	emitAssets(compilation, callback) {
		...
		console.log('  2.8.============================> [生命钩子]compiler.onCompiled 方法执行emit')
		this.hooks.emit.callAsync(compilation, err => {
			if (err) return callback(err);
			outputPath = compilation.getPath(this.outputPath);
			console.log('    2.8.1.============================> 创建输出文件夹')
