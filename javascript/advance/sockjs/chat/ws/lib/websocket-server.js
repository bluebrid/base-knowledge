...
class WebSocketServer extends EventEmitter {
  ...
  constructor(options, callback) {
    ...
    if (this._server) {
      // 如果传入的是一个server 则自动的给添加了三个事件，否则需要手动的添加对应的事件
  ...
  completeUpgrade(key, extensions, req, socket, head, cb) {
    ...
    // 根据sec-websocket-key 和 GUID生成新Sec-WebSocket-Accept 作为头部返回给请求
    ...
    // socket 是req.socket 的内置对象
    socket.write(headers.concat('\r\n').join('\r\n'));
    socket.removeListener('error', socketOnError);
    // ws是我们自定义的Websocket 对象 const ws = new WebSocket(null);
    ws.setSocket(socket, head, this.options.maxPayload);

    if (this.clients) {// 将每一个连接都保存在
      this.clients.add(ws);
