...
(function (global, factory) {
...

  {
    ...
    if (typeof window !== 'undefined' && typeof window.dispatchEvent === 'function' && typeof document !== 'undefined' && typeof document.createEvent === 'function') {
      ...
      var invokeGuardedCallbackDev = function (name, func, context, a, b, c, d, e, f) {
        log(`(invokeGuardedCallbackDev) func: ${JSON.stringify(func)}`)
        ...
        var funcArgs = Array.prototype.slice.call(arguments, 3);
        log(`(invokeGuardedCallbackDev) fakeNode.addEventListener(evtType, callCallback, false);`)
        function callCallback() {
          ...
          log('真正执行注册事件的handler, func 就是commitRoot 方法中传递过来的，如： commitBeforeMutationLifecycles， commitAllHostEffects， commitAllLifeCycles', 'red')
          func.apply(context, funcArgs);
          didError = false;
        }
        ...
        log(`============================================>fackNode： <react></react>
        初始化事件， 并且发射事件evt.initEvent fakeNode.dispatchEvent
      `)
  ...
  function forEachAccumulated(arr, cb, scope) {
    log(`(forEachAccumulated)执行注册的事件`)
  ...
  function trapBubbledEvent(topLevelType, element) {
    ...
    log(`(trapBubbledEvent) addEventBubbleListener 也就是调用： element.addEventListener(eventType, listener, false);`)
    addEventBubbleListener( // element, eventType, listener
      element,
      getRawEventName(topLevelType),// eventType
      // Check if interactive and wrap in interactiveUpdates
      dispatch.bind(null, topLevelType)// handler
    );
  }
  ...
  function setInitialDOMProperties(tag, domElement, rootContainerElement, nextProps, isCustomComponentTag) {
    ...
    for (var propKey in nextProps) {
      ...
      if (propKey === STYLE$1) { // style
        ...
        log(`(setInitialDOMProperties)对suppressContentEditableWarning 和suppressContentEditableWarning的处理`)
        // Noop
      } else if (propKey === AUTOFOCUS) {
        log(`(setInitialDOMProperties)对autoFocus的处理`)
  ...
  function updateDOMProperties(domElement, updatePayload, wasCustomComponentTag, isCustomComponentTag) {
    ...
    log(`(updateDOMProperties)真正去更新Dom Node,
  updatePayload是一个数组， 长度只是为2，对应属性名称， 已经属性的值，如: ["children", "Click Me(2)"],`)
  ...
  function updateProperties(domElement, updatePayload, tag, lastRawProps, nextRawProps) {
    ...
    // Apply the diff.
    log(`(updateProperties)直接应用diff 到Dom 上，改变UI`)
  ...
  function commitUpdate(domElement, updatePayload, type, oldProps, newProps, internalInstanceHandle) {
    ...
    log(`(commitUpdate)将diff 应用到Dom node`)
  ...
  function createFiberRoot(containerInfo, isConcurrent, hydrate) {
    log(`(createFiberRoot)创建fiberRoot 挂载在ReactRoot上`)
     ...
     * 并且父节点只有一个 child 指向它的第一个子节点，即便是父节点有好多个子节点。
     * 那么多个子节点如何连接起来呢？答案是 sibling，每个子节点都有一个 sibling 属性指向着下一个子节点，
     * 都有一个 return 属性指向着父节点。这么说可能有点绕，我们通过图来了解一下这个 fiber 树的结构。
     */
    log(`(createFiberRoot)current是一个FiberNode对象`)
    var root = void 0; // root 
  ...
  var classComponentUpdater = {
    ...
    enqueueSetState: function (inst, payload, callback) {
      log(`(enqueueSetState) setState 调用enqueueSetState`)
      var fiber = get(inst); // 获取组件对应的_reactInternalFiber, 也就是一个FiberNode对象
      /**
       * function get(key) {
            return key._reactInternalFiber;
         }
      ...
      log(`(enqueueSetState) 其实和ReactDOM.render 方法中执行scheduleRootUpdate的逻辑一样, 
    但是在合成事件中， isRendering, isBatchingUpdates, isUnbatchingUpdates三个值为false, false, true,
    导致在requestWork中被return, 并没有走到performWorkOnRoot或者是performSyncWork, 
    return 后的代码会回到你的click event 执行其他的代码，也就是this.setState的回调函数，
    因为上面并没有走到performWork去更新，知道代码执行完成， 整个在interactiveUpdates$1中的try代码块也就执行完了.
    接下来就会执行interactiveUpdates$1中的finally 代码块
    `)
      flushPassiveEffects();
      log(`(enqueueSetState) 设置update, 这个对象如下:
    return {
      expirationTime: expirationTime,
  
      tag: UpdateState,
      payload: { // 是setState 传入的partialState
        count: 2
      },
      callback: fn,// 是setState 的回调函数
    ...
    `)
      log(`enqueueUpdate 将updae 绑定在fiber.updateQueue 上面
    queue = fiber.updateQueue
    queue.lastUpdate.next = update;
    queue.lastUpdate = update;
    `)
      // var obj = new Proxy(update, {
  ...
  function commitUpdateEffects(effect, instance) {
    while (effect !== null) {
      ...
      log(`(commitUpdateEffects) 提交update 受影响的功能，effect.callback获取的就是我们setState的回调函数 `)
      if (_callback3 !== null) {
        effect.callback = null;
        log(`(commitUpdateEffects)执行setState 的回调函数`)
      ...
      case ClassComponent:
        {
          ...
          if (updateQueue !== null) {
            ...
            // but instead we rely on them being set during last render.
            // TODO: revisit this when we implement resuming.
            log(`(commitLifeCycles)提交setState受影响的函数`)
  ...
  function commitWork(current$$1, finishedWork) {
    log(`(commitWork)finishedWork 就是nextEffect， 而且nextEffect是一个全局变量`)
    if (!supportsMutation) { // 直接跳过这个逻辑分支
      switch (finishedWork.tag) {
        case FunctionComponent:
        case ForwardRef:
        case MemoComponent:
      ...
      case HostComponent:
        {
          ...
          if (instance != null) {
            ...
            // this case.
            /**
             current$$1 是一个FiberNode,current$$1.memoizedProps是和上面的newProps一样的对象， 
             oldProps: 也是缓存老的属性，如： 
             {
                children: 'Click Me(1)',
                onClick: f ()
              }
              newProps 和 oldProps的区别是children 的内容不一样了
             */
            var oldProps = current$$1 !== null ? current$$1.memoizedProps : newProps;
            var type = finishedWork.type;
            // TODO: Type the updateQueue to be specific to host components.
            var updatePayload = finishedWork.updateQueue;
            // updatePayload 是一个数组： ["children", "Click Me(2)"]
            finishedWork.updateQueue = null;
            if (updatePayload !== null) {
              log(`(commitWork)更新组件UI, instance就是对应的Dom`)
  ...
  function commitAllHostEffects() {
    while (nextEffect !== null) {
      ...
      switch (primaryEffectTag) {
        case Placement:
          {
            log(`(commitAllHostEffects) ReactDOM.render 第一次render 会调用这个方法，然后去变更UI`)
  ...
  function commitAllLifeCycles(finishedRoot, committedExpirationTime) {
    {
      log(`(commitAllLifeCycles) 提交所有的生命周期 `)
  ...
  function renderRoot(root, isYieldy) {
    ...
    // Ready to commit.
    log(`(renderRoot)调用onComplete`)
    log(`(renderRoot) rootWorkInProgress.firstEffect.memoizedProps 已经有了需要改变的props, 这个rootWorkInProgress 也就是后续使用的: finishedWork`)
    /**
  ...
  function interactiveUpdates$1(fn, a, b) {
    ...
    if (!isBatchingUpdates && !isRendering && lowestPriorityPendingInteractiveExpirationTime !== NoWork) {
      ...
      log(`(interactiveUpdates$1)由事件触发，然后执行performWork`)
      performWork(lowestPriorityPendingInteractiveExpirationTime, false);
      lowestPriorityPendingInteractiveExpirationTime = NoWork;
    }
    var previousIsBatchingUpdates = isBatchingUpdates;
    log(`(interactiveUpdates$1)合成事件， 设置isBatchingUpdates为true， 所以在20168行requestWork方法会直接retrun,
  所以在执行setState后获取不到最新的state`)
    /**
  ...
  function requestWork(root, expirationTime) {
    ...
    isBatchingUpdates = true;
    try {
      log(`(interactiveUpdates$1) 从dispatchEvent 到requestWork 的调用栈就是属于try代码块`)
      return unstable_runWithPriority(unstable_UserBlockingPriority, function () {
        return fn(a, b);
      });
    } finally {
      isBatchingUpdates = previousIsBatchingUpdates;
      log(`(interactiveUpdates$1) 执行事件的finally -> performSyncWork, 更新到UI的一个过程。。。。。(important)`, '#8B1A1A')
      if (!isBatchingUpdates && !isRendering) {
        // 执行同步工作
        performSyncWork();
      }
  ...
  function updateContainerAtExpirationTime(element, container, parentComponent, expirationTime, callback) {
    ...
    log(`(updateContainerAtExpirationTime)执行scheduleRootUpdate方法`)
  ...
  ReactRoot.prototype.unmount = function (callback) {
    log(`(ReactRoot)调用ReactRoot 的 unmount 方法`)
    var root = this._internalRoot;
    var work = new ReactWork();
    callback = callback === undefined ? null : callback;
    {
  ...
  ReactRoot.prototype.legacy_renderSubtreeIntoContainer = function (parentComponent, children, callback) {
    log(`(ReactRoot)调用ReactRoot 的 legacy_renderSubtreeIntoContainer 方法`)
    var root = this._internalRoot;
    var work = new ReactWork();
    callback = callback === undefined ? null : callback;
    {
  ...
  ReactRoot.prototype.createBatch = function () {
    log(`(ReactRoot)调用ReactRoot 的 createBatch 方法`)
