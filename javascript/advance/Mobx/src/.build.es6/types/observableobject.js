...
export class ObservableObjectAdministration {
    ...
    addObservableProp(propName, newValue, enhancer = this.defaultEnhancer) {
        ...
        // 并且将对应的属性直接挂载在target 上面, 但是对get 和set进行了拦截
        /**
...
export function asObservableObject(target, name = "", defaultEnhancer = deepEnhancer) {
    ...
    /**
     * 创建一个ObservableObjectAdministration对象
     *     
     * constructor(target, values = new Map(), name, defaultEnhancer) {
        this.target = target;
        this.values = values;
        this.name = name;
        this.defaultEnhancer = defaultEnhancer;
        this.keysAtom = new Atom(name + ".keys"); // 生成了一个Atom(原子)的对象
      }
     */
   ...
   /**
    * target： 初始化是一个{}空对象
    * $mobx: 是一个Symbol(mobx administration)
    * adm: 是上面生成的一个ObservableObjectAdministration对象
    * 
