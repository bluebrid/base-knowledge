...
export function startBatch() {
    // globalState 就是在.build.es6/core/globalstate.js 中维护的全局的变量对象
    globalState.inBatch++;
}
...
export function reportObserved(observable) {
    ...
    if (derivation !== null) {
        if (derivation.runId !== observable.lastAccessedBy) {
            ...
            // derivation.onInvalidate就是我们在
            /**
             * derivation.onInvalidate 就是我们创建Reaction 对象的第二个参数， 而view 就是我们运行autorun 的参数
...
export function propagateChanged(observable) {
    ...
    //observable 是一个 ObservableValue对象
