...
3. 可以利用**preload**去实现加载JS脚本， 不会阻塞DOM解析， 如： `<link rel="preload" href="/main.js" as="script">`(放在header标签中越早也好)， 这个的意思， 预先加载资源，并且把资源保存在**内存** 中， 只有当遇到**script**标签也为**main.js**的时候，浏览器才会直接将预先加载的JS去执行掉.
